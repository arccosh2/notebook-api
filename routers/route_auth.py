...
from schemas import Csrf, UserBody, UserInfo, SuccessMessage
...
async def login(requset: Request, response: Response, user: UserBody, csrf_protect: CsrfProtect = Depends()):
  csrf_token = csrf_protect.get_csrf_from_headers(requset.headers)
  csrf_protect.validate_csrf(csrf_token)
  user = jsonable_encoder(user)
